...
function baseState({ castleLevel, troops, dailyQuests, mailbox }) {
  return {
    ...
    dailyQuests: dailyQuests || { resetAt: Date.now() + 20 * 60 * 1000, progress: { login: 1, goldProduced: 0, monstersKilled: 0, troopsTrained: 0, heroesRecruited: 0 }, claimed: {} },
    mailbox: mailbox || [],
  };
}
...
async function main() {
  ...
  await putState(tokenA, baseState({
    ...
    dailyQuests: { resetAt: Date.now() + 20 * 60 * 1000, progress: { login: 1, goldProduced: 0, monstersKilled: 3, troopsTrained: 0, heroesRecruited: 0 }, claimed: {} },
  }));
