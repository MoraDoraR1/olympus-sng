...
### 바로 둘러볼 수 있는 테스트 계정 만들기

신규 계정은 성 Lv.1·자원 80/80/60/150로 시작해서 정복 맵(성 Lv.5+ 필요)조차 못 본다.
에뮬레이터가 떠 있는 상태에서 아래를 실행하면 바로 둘러볼 수 있는 계정이 만들어진다
(닉네임 `테스트유저` / 비밀번호 `test1234`, 성 Lv.10, 자원 각 15,000, 병사 25명, 정복 맵
참가 완료, 아이템 2개 보유):

```bash
node scripts/dev-test/create-seed-account.mjs
```

이 스크립트는 에뮬레이터 안에만 계정을 만든다 — 에뮬레이터를 재시작하면(기본적으로
데이터가 저장되지 않으므로) 다시 실행해야 한다. 브라우저에서 `index.html`을 열어 이
닉네임/비밀번호로 로그인하면 된다.
